         ...
         <table width="1000px;" height=200px; align=center style="background:#E6E6E6; border-radius:10px 30px 50px 80px;margin:0 auto;">
            ...
            <tr height=60px align=right>
               <td align=right colspan=5>적립되는 마일리지:<input type=text class=a id=o_mile size=5 readonly style="border:none; "><br>
                                    ...
                                    사용하실 마일리지:<input type=text class=d id=using_mile size=5 placeholder=입력><br>
                                    <input type=button id=use class=btngo1 value=사용하기>&nbsp;<input type=button class=btngo1 id=reset value=취소></td>
                                    </tr>
            <tr height=60px align=right>
               <td align=right colspan=5 class=b >지불 금액:<input type=number name="${total}" id="tot" class=c min=0 readonly></td>
            </tr>
            </table>
            <br><br>
                <table width="1000px;" height="600px;" align="center" style="background:#E6E6E6; border-radius:10px 30px 50px 80px;margin:0 auto;"> 
                    <tr> 
                        ...
                        <td width="500px" id="orderInfo">
                            ...
                            &nbsp;<input type="text" id="memberName" class=textbox value="${order.member_name}"><br><br>
                            &nbsp;<input type="text" id="addrCode" class=textbox size="5;">&nbsp;<input type="button" onclick="click()" class=btngo id="postCode" class=btngo value="우편번호"><br><br>
                        ...
                        <td width="500px" id="postInfo">
                            ...
                            <p align="right">
                                <input type="button" value="pay" class=btngo id=pay align="right">&nbsp;
                                <input type="button" value="취소" class=btngo id=cencle align="right">
                            </p>
                        </td>
                    </tr>  
            </table>
...
<script type="text/javascript">
    ...
    .on('click','#pay',function(){
       ...
       if($('#memberName').val()==""){
          ...
          if(a==false){
             ...
             console.log(addr);
             $(location).attr("href","pay_complete?cart_id="+$('#cart_id').val()+"&addr="+addr+"&total="+$('#hi').val()+
    ...
    .on('click','#use',function(){
       ...
       var basemile =$('#use_mile').val() //회원님의 남은 마일리지
       var usemile = $('#using_mile').val() //사용하실 마일리지 
       
       var totaling = parseInt(total)-parseInt(usemile); //총합계 - 쓸마일리지
       var totalmile = parseInt(basemile)-parseInt(usemile); //기존마일리지 - 쓸마일리지
       
       if(usemile==''){ 
          alert("값을 입력하세요"); //사용할마일리지에 값이 없으면 alert
          return false;
       }
       
       var a=$('#use_mile').val();//기존마일리지
       var b=$('#using_mile').val();//사용할마일리지
       if(parseInt(a)<parseInt(b) || parseInt(total)<parseInt(b)){
          alert("사용가능한 마일리지 범위를 초과하셨습니다"); //기존마일리지와 사용할마일리지 비교해주기 
          return false;
          
       } else{
          $('#tot').val(totaling); //총합계-쓸마일리지 총합값에 넣어주기  
          $('#use_mile').val(totalmile); //기존마일리지-쓸마일리지 =기존마일리지값에 넣어주기
          var save_mile = $('#tot').val()/100; //마일리지 적립해주는 값지정하기
          $('#o_mile').val(Math.ceil(save_mile)); //마일리지 적립해주기 (소수값 들어가지않게)
