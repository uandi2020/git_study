        ...
        <!-- 테이블 넣는곳 시작부분 ^^-------------------------------------------------------------------------------------------------------- -->
         <!-- 여기에 넣어주고 TABLE인 경우 style='margin:0 auto; 넣어주면 가운데정렬됨 -->
                  <input type="hidden" name="m_id" value="${content_view.m_id}">
                    ...
                    <tr>
                        <td align='right' colspan="3"><input type='button' class=btn id=modify value='수정'>&nbsp;&nbsp;<input type='button' class=btn id=list value='목록보기'>&nbsp;&nbsp;<input type='button' class=btn id='del1' value='삭제'></td>
                    </tr>
                </table>
        <!-- 테이블 끝나는 시작부분 ^^-------------------------------------------------------------------------------------------------------- -->
    </nav>
...
.on('click','#fqa_btn1',function(){
   $(location).attr('href',"fqa_update?fqa_id="+$('#fqa_id').val()+"&group="+$('#dab').val())
})
