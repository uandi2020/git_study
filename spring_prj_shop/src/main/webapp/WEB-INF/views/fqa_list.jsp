...
<div id=toptop style='position:relative;float:left'>
  <nav class=title_nav>
    ...
    <nav style='margin-top:50px;margin-bottom:300px;width:1920px'>
        <!-- 테이블 넣는곳 시작부분 ^^-------------------------------------------------------------------------------------------------------- -->
         <!-- 여기에 넣어주고 TABLE인 경우 style='margin:0 auto'; 넣어주면 가운데정렬됨 -->
               <div>
                  <div align=center style='font-size:28px;margin:30px 30px;'>FQA</div>
                        <table class="fqa_tbl" style='margin:0 auto; text-align:center;' >
                        ...
                        <c:forEach items="${fqa}" var="dao">
                        <tr onmouseover="this.style.background='#EAE6E6';" onmouseout="this.style.background='white';" >
                            <td align=center>${dao.rownum}</td>
                            <td>${dao.fqa_name}</td>
                            <td><a style='color:black' href="fqa_content_view?fqa_id=${dao.fqa_id}&m_id=${dao.m_id}" onmouseover="this.style.color='grey';" onmouseout="this.style.color='black'">${dao.fqa_title}</a></td>
                        </tr>
                        </c:forEach>
                        <tr>
                        <td></td>
                           <td style='text-align:right;'>
                              ...
                              <a href="fqa_list?pageNum=${back}">[이전]</a>
                              </c:if>
                              <ul class=pageNum>
                              <c:forEach items='${num}' var='n'>
                               <li><a href='fqa_list?pageNum=${n}' id="${n}">${n}</a></li>
                            </c:forEach>
                            </ul>
                            <c:if test="${endno eq 1}">
                            <a href="fqa_list?pageNum=${nexts}">[다음]</a>
                           </c:if>
                           </td>
                            <td style='float:right; width:100px'>
                               <ul> 
                              <li><a class=v href="fqa_write_view">글쓰기</a></li>
                               </ul>
                            </td>
                        </tr>
                        </table>
   ...
   <script type="text/javascript">initMoving(document.getElementById("gotop"), 700, 50, 50);</script>
</body>
<script>
$(document)
