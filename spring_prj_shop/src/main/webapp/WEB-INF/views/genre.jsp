    ...
    <nav style='margin-bottom:300px;width:1920px'>
        <!-- 테이블 넣는곳 시작부분 ^^-------------------------------------------------------------------------------------------------------- -->
         <!-- 여기에 넣어주고 TABLE인 경우 style='margin:0 auto;' 넣어주면 가운데정렬됨 -->
                ...
                <div>
                    <div class=content_div>장르별</div>
                    <div class=kate2><a href='genre?pageNum=1'>전체 카테고리</a></div>
                        <div class=kate3>
                        <ul class=kate4>
                            <li><a href='genresel?pageNum=1&genre=가요'>가요</a></li><li><a href='genresel?pageNum=1&genre=Pop'>Pop</a></li><li><a href='genresel?pageNum=1&genre=OST'>OST</a></li><li><a href='genresel?pageNum=1&genre=Ballad'>Ballad</a></li>
                        </ul>
                    </div>
                </div>
                <div class=score_all>
                    <table class="tbl" align="center">
                        ...
                        <c:forEach items="${score}" var="s">
                           <tr id='${s.s_id}' onmouseover="this.style.background='#EAE6E6'" onmouseout="this.style.background='white'" onclick="location.href ='product?s_id=${s.s_id}';">
                              <td>${s.rownum}</td><td><img src="resources/image/${s.s_id}.gif" alt="My Image"></td><td>${s.score_name}</td><td>${s.score_singer}</td><td>${s.score_price}</td><td>${s.score_parts}</td><td id=tdtd>${s.score_genre}</td>
                           </tr>
                        </c:forEach>
                         <tr>
                           <td style='float:center;' colspan=7>
                              <c:if test="${startyes eq 1}">
                              <a href="genre?pageNum=${back}">[이전]</a>
                              </c:if>
                                <ul class=pageNum>
                              <c:forEach items='${num}' var='n'>
                               <li><a href='genre?pageNum=${n}' id="${n}" class="${n}">${n}</a></li>
                            </c:forEach>
                            </ul>
                            <c:if test="${endno eq 1}">
                            <a href="genre?pageNum=${nexts}">[다음]</a>
                           </c:if>
                           </td>
                             </tr>                
                        </tbody>
...
$(document)
.ready(function(){
// 	?zzzzzzzzz뇌정지 다시 정리하고올게
// 			이치문어케씀 기억안남
// 		이거 아닌가???????????오 뭔가맞아>>??아맞네 스미마셍 뭔데.....생각대로 햇그
// 		그 이치문 안에 스위치문 넣으면 안되나>?? 해봐 ㄱㄱ
// 				어떻게썼어
// 	ee=true;
// 	$('#tdtd').each(function(){
// 		console.log($(this).text());
// // 		console.log('ndx ['+ndx+'] dd ['+dd+']')
// 	})
	a=$('tbody tr:eq(0) td:eq(6)').text()
	b=$('tbody tr:eq(3) td:eq(6)').text()
// 	console.log(a)
// 	console.log(b)
// 	console.log("안녕"+$('#tdtd').text())
	if(a==b){
		switch($('#tdtd').text()){
		case '가요':
