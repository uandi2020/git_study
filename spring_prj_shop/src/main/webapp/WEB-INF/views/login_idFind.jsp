    ...
    <nav style='margin-top:100px;margin-bottom:350px;width:1920px'>
        <!-- 테이블 넣는곳 시작부분 ^^-------------------------------------------------------------------------------------------------------- -->
         <!-- 여기에 넣어주고 TABLE인 경우 style='margin:0 auto;' 넣어주면 가운데정렬됨 -->
                <form action="idInfo" method="POST" id=frm>
                <table class=login_table align='center' valign='middle' style='margin:0 auto;'>
                    <tr><th colspan=3 class=log><h1>find your ID</h1></th></tr>
                    <tr><td colspan=3><input type=hidden name=hide></td></tr>
                    <tr><td colspan=3><hr><br></td></tr>
                    <tr><td colspan=3 align=center><input type=text id=loginName_id name=loginName class=login_box placeholder="Name"></td></tr>
                    <tr><td colspan=3></td></tr>
                    <tr><td colspan=2 align=left><input type=text id=emailId_id name=emailId class=login_box2 placeholder="Email">@
                    <input type=text id=emailAdress_id name=emailAdress class=login_box2></td>
                    ...
                    <tr><td align=center colspan=3><br><input type=button id=btn2 value="확인" class=login_button><input type=button id=btn1 value="취소" class=login_button></td></tr>
...
//       var ad=$('#adress_id option:selected').val();
//        //console.log(ad);
//       $('#emailAdress_id').val(ad);
      $('#adress_id').each(function(){
          var eachAddr=$(this).val();
          var s=$('#emailAdress_id').val(eachAddr);
          console.log(s);
          if(eachAddr=="직접입력"){
              $('#emailAdress_id').val("");
              document.getElementById('emailAdress_id').focus();
   ...
   .on('click','#btn2',function(){
      if($('.login_box').val()==''){
         alert('이름을 입력하세요.');
      }else if($('.login_box2').val()==''){
         alert('이메일을 입력하세요.');
      }else{
//          $('#frm').submit();
//          var email=$('#emailId_id').val()+'@'+$('#emailAdress_id').val();
         ...
         $.ajax({
            ...
            datatype:'text',
            url:'idFind',
            data:{name:$('#loginName_id').val(),emailId:$('#emailId_id').val(),
               emailAdress:$('#emailAdress_id').val()},
            beforeSend:function(){console.log(this.data)},
            success:function(str){
               console.log(str);
               if(str==0){
                  alert('입력된 이름과 이메일로 등록된 아이디가 없습니다.')
               }else{
                  $('input[name=hide]').val(str);
                  $('#frm').submit();
               }
