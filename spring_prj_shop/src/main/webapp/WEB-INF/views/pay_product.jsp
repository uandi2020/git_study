        ...
        <!-- 테이블 넣는곳 시작부분 ^^-------------------------------------------------------------------------------------------------------- -->
         ...
         <table width="1000px;" height=200px; align=center style="background:#E6E6E6; border-radius:10px 30px 50px 80px;margin:0 auto;">
            ...
            <tr height=100px align=center >
               <td><img class="coverImg" src="resources/image/${pay.s_id}.gif"></td>
               <td>${pay.score_name}</td>
               <td>${pay.score_price}</td>
               <td>${cnt}</td>
               <td>${total}<input type=hidden name='hih' id='hi' value='${total}'></td>
            </tr>
            <tr height=60px align=right>
               <td align=right colspan=5>적립되는 마일리지:<input type=text class=a id=o_mile size=5 readonly style="border:none; "><br>
                                    사용가능한 마일리지:<input type=text class=a name='${mile.mileage}' id=use_mile size=5 value='${mile.mileage}' readonly style="border:none"><br>
                                    사용하실 마일리지:<input type=text class=b id=using_mile size=5 placeholder=입력><br>
                                    <input type=button id=use value=사용하기 class=btngo1>&nbsp;<input type=button id=reset value=취소 class=btngo1></td>
                                    </tr>
            <tr height=60px align=right>
               <td align=right colspan=5 class=z>지불 금액:<input type=number class=c name="${total}" id="tot" min=0 value="${total}" readonly></td>
            </tr>
            </table>
            <br><br>
                <table width="1000px;" height="600px;" align="center" style="background:#E6E6E6; border-radius:10px 30px 50px 80px;margin:0 auto;"> 
                    <tr> 
                        ...
                        <td width="500px" id="orderInfo">
                            주문정보<br><br>
                            <input type='hidden' id=addrs value='${order.member_adress}'>
                            <input type='hidden' id=mobile value='${order.member_mobile}'>
                            <input type='hidden' id=email value='${order.member_mail }'>
                            &nbsp;<input type="text" class=textbox id="memberName" value="${order.member_name}"><br><br>
                            &nbsp;<input type="text" class=textbox id="addrCode" size="5;">&nbsp;<input type="button" class=btngo onclick="click()" id="postCode" value="우편번호"><br><br>
                            &nbsp;<input type="text" class=textbox id="addr1"><br><br>
                            &nbsp;<input type="text" class=textbox id="addr2"><br><br>
                            &nbsp;<input type="phone" size=3; class=textbox id=mobile1>-<input type="phone" class=textbox id="mobile2" size="5;">-<input type="phone" class=textbox id="mobile3" size="5;"><br><br>
                            &nbsp;<input type="text" id="email1" class=textbox size="8">&nbsp;@&nbsp;<input type="text" class=textbox id="email2" size="8">
                        ...
                        <td width="500px" id="postInfo">
                            ...
                            &nbsp;<input type="text" class=textbox id="addrCode1" size="5;">&nbsp;<input type="button" class=btngo id="postCode2" value="우편번호"><br><br>
...
<script type="text/javascript">
    $(document).ready(function(){
       ...
       var save_mile = $('#tot').val()/100;   
       $('#o_mile').val(save_mile);
       console.log(save_mile)
    ...
    .on('click','#pay',function(){
       ...
       if($('#memberName').val()==""){
          ...
          if(a==false){
             ...
             $(location).attr("href","pay_complete2?s_id="+$('#s_id').val()+"&addr="+addr+"&total="+$('#hi').val()+
           "&basemile="+$('#use_mile').val()+"&point="+$('#o_mile').val()+"&usingmile="+$('#using_mile').val());  
           }
       }
    })
    ...
    .on('click','#use',function(){
       var total = $('#tot').val() //총합계
       var basemile =$('#use_mile').val()
       var usemile = $('#using_mile').val() //쓸마일리지 
       
       var totaling = parseInt(total)-parseInt(usemile); //총합계 - 쓸마일리지
       var totalmile = parseInt(basemile)-parseInt(usemile); //기존마일리지 - 쓸마일리지
       
       
       var a=$('#use_mile').val();
       var b=$('#using_mile').val();//사용할
       if(parseInt(a)<parseInt(b) || parseInt(total)<parseInt(b)){
          alert("사용가능한 마일리지 범위를 초과하셨습니다");
          return false;
          
       } else{
          $('#tot').val(totaling);
          $('#use_mile').val(totalmile);
          var save_mile = $('#tot').val()/100;   
          $('#o_mile').val(Math.ceil(save_mile));
       
       }
    })
    
    .on('click','#reset',function(){
       console.log($('#o_mile').val())
       $('#tot').val($('#tot').attr('name'));
       $('#use_mile').val($('#use_mile').attr('name'));
       $('#using_mile').val(0);
       var save_mile = $('#tot').val()/100;   
       $('#o_mile').val(save_mile);
       console.log($('#o_mile').val())
